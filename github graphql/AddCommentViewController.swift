...
class AddCommentViewController: UIViewController, UITextViewDelegate {
    ...
    @IBAction func add(_ sender: UIBarButtonItem) {
        if textView.text.isEmpty {
            ...
            apollo.perform(mutation: GitHubSchema.AddCommentMutation(id: issueId, body: textView.text)) { result in
                switch result {
                case .success(let value):
                    let alert = UIAlertController(title: nil, message: "追加されました", preferredStyle: .alert)
