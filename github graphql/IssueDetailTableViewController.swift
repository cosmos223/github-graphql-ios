...
class IssueDetailTableViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        apollo.fetch(query: GitHubSchema.GetIssueAndCommentsByIDQuery(id: issueId, after: nil)) { [self] result in
            ...
            if let canDelete = data.node?.asIssue?.viewerCanDelete, canDelete {
                elements.append(UIAction(title: "Delete Issue", image: UIImage(systemName: "trash"), handler: { _ in
                    ...
                    let okAction = UIAlertAction(title: "はい", style: .destructive) { [self] _ in
    ...
    */
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if segue.identifier == "toAddComment" {
            ...
            vc.issueId = issueId
        } else if segue.identifier == "toEditIssue" {
            guard let navC = segue.destination as? UINavigationController,
